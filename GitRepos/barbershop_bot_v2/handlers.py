...
async def cmd_start(message: Message):
    ...
    welcome_img = os.path.join(os.path.dirname(__file__), "welcome.png")
...
def _get_master_panel_keyboard(master_id: int = None, telegram_id: str = None) -> InlineKeyboardMarkup:
    ...
    # WebApp panel button (shown first if we have the needed info)
    if master_id and telegram_id:
        try:
            base_url = settings.WEBAPP_URL.replace('/webapp', '')
            webapp_url = f"{base_url}/master-webapp?master_id={master_id}&uid={telegram_id}"
            builder.row(InlineKeyboardButton(
                text="📱 Відкрити WebApp панель",
                web_app=WebAppInfo(url=webapp_url)
            ))
        except Exception:
            pass  # If URL construction fails, skip WebApp button
